/* ==========================================================================
   ...
   字重與字級刻意壓得很低（細體、16px），因為在這種透明度下「看得見的份量」是由
   墨跡面積決定的，不是由顏色深淺；想耐壓縮就得犧牲這個，把 MARK_ALPHA 拉高
   並同時加粗。
...
/* 字級與字重：越小越細越不明顯，代價是拉對比之後比較難讀 */
const FONT_SIZE = 16;
const FONT_WEIGHT = 400;

/*
 * 一行最多幾個「半形寬度」：非 ASCII 算 2，其餘算 1。磚寬 260、字級 16、
 * 置中對齊，24 個半形寬度還留得下邊（IP 如 192.0.2.17 是 15，塞得進去）。
 */
...
/* 各行基線的位置（磚高 240，整塊垂直置中） */
const ROW_Y = [70, 108, 146, 184];

/* 磚的尺寸：越小鋪得越密，但字也越小越容易被壓縮吃掉；
   高度要能裝下四行轉 -20 度之後的範圍（轉完約 197px） */

...
每一張**送進房間的圖片**在轉檔前都會被打上盲水印（`assets/js/watermark.js`，演算法移植自 [blind_watermark](https://github.com/guofei9987/blind_watermark) 的 DWT→區塊 DCT→SVD→QIM 流程）：

- 標記內容＝上傳者的指紋（FNV-1a 32 位元，base36）加上日期，例如 `wd64d9.ft2`。
- 強制開啟，沒有任何介面開關或說明文字；取出也不需要原圖。
- 量到的效果：肉眼差異 SSIM 約 0.98～0.99（大面積平色區會被跳過，不留方塊感），重新壓縮成 WebP q82／q60 之後仍取得回來（`npm run test:watermark`）。
- **已知限制**：被重新縮圖或裁切之後取不回來（區塊格線對不上）；截圖也不可能帶走標記——瀏覽器裡沒有「整頁像素都歸我們管」這種事，試過疊一層 canvas，數學上不可行。
- 取出工具：`node tools/watermark-decode.mjs 圖檔 [成員名單檔]`。
- 它能指出「這張圖是誰上傳的」，不能阻止外流，也不能證明截圖者是誰。

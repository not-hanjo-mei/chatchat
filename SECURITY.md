...
三層，內容都是「**使用者 ID 指紋 ＋ 裝置簽章**」，例如 `wd64d9.SM-G991B-3k9v2xq`。ID 用 FNV-1a 壓成 6~7 個 base36 字元才塞得進標記；裝置簽章是用 canvas 畫一段固定圖樣再雜湊像素結果（字型、反鋸齒、GPU、色彩處理都會影響輸出），不用 user agent——UA 可以造假、會被隱藏，也會隨版本變動。

1. **看得見的畫面浮水印**（`assets/js/screenmark.js`）：九段小字「暱稱 · 裝置」在畫面上緩慢飄動，`pointer-events: none` 不影響操作。截圖、翻拍都會帶著它。
...
| 圖片本身再壓成 WebP q82 | 取得回 |
| 圖片被裁切或外面被加了一圈邊框 | 取得回 |
| 文字被清掉零寬字元（例如再次貼回本系統的輸入框） | 取不回 |
| 截圖被重新壓縮成 JPEG | 浮水印仍在（看得見），但文字／圖片標記要另外取 |

...
兩層，內容都只有**裝置標籤**：機型 ＋ 裝置指紋，例如 `SM-G99-3k9v2`。裝置指紋是用 canvas 畫一段固定圖樣再雜湊像素結果（字型、反鋸齒、GPU、色彩處理都會影響輸出），不用 user agent——UA 可以造假、會被隱藏，也會隨版本變動。不放使用者 ID：裝置指紋就能對到人。

1. **訊息文字裡的隱形標記**（`watermark.js` 的 `embedTextMark`）：文字進畫面時在結尾插入零寬字元（長訊息再多一份在中段標點後），看不出來、不影響搜尋與排版，但複製貼上會一起帶走。資料庫裡存的原字串不變。
2. **送進房間的圖片**：轉檔前寫入像素盲水印（DWT→區塊 DCT→SVD→QIM，三通道、128 bits 負載、2D 鋪磚），取出時會搜尋格線相位與倍率。

畫面上沒有任何可見的水印，也沒有開關或說明文字。取出工具：

```bash
node tools/watermark-decode.mjs 圖檔
node tools/watermark-decode.mjs --text 文字檔
```
...
- SSIM 約 0.95～0.98（強度與耐受度的取捨表在 `assets/js/watermark.js` 的 `STEP_MAIN` 註解）。
- 它能指出「這段文字或這張圖是哪台裝置留下的」，不能阻止外流，也不能證明是誰按下的快門。

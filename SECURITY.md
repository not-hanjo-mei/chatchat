...
三層，內容都只有**裝置標籤**：機型 ＋ 裝置指紋，例如 `SM-G99-3k9v2`。裝置指紋是用 canvas 畫一段固定圖樣再雜湊像素結果（字型、反鋸齒、GPU、色彩處理都會影響輸出），不用 user agent——UA 可以造假、會被隱藏，也會隨版本變動。不放使用者 ID：裝置指紋就能對到人，畫面上也看得懂。

1. **看得見的畫面浮水印**（`assets/js/screenmark.js`）：九段小字（就是裝置標籤）在畫面上緩慢飄動，`pointer-events: none` 不影響操作。截圖、翻拍都會帶著它。
2. **訊息文字裡的隱形標記**（`watermark.js` 的 `embedTextMark`）：文字進畫面時在結尾插入零寬字元（長訊息再多插一份在中段標點後），看不出來、不影響搜尋與排版，但複製貼上會一起帶走。資料庫裡存的原字串不變。
3. **送進房間的圖片**：轉檔前寫入像素盲水印（`watermark.js`，DWT→區塊 DCT→SVD→QIM，三通道取多數決），耐 WebP q82 重新壓縮。

取出工具：`node tools/watermark-decode.mjs 圖檔 [名單檔]` 或 `node tools/watermark-decode.mjs --text 文字檔 [名單檔]`，兩種都會試。
...
| --- | --- |
| 畫面浮水印 | 肉眼可讀，截圖即可辨識 |
| 訊息文字被複製、貼到別的檔案、前後加了字 | 取得回 |
| 圖片重新壓成 WebP q82 / JPEG | 取得回（WebP q60 這種重壓取不回） |
| 圖片外圈被加了一圈、被裁掉 30% | 取得回（格線相位與 2D 鋪磚位移都會搜尋） |
| 圖片被放大／縮小 3% | 取得回（工具會掃倍率） |
| 文字被清掉零寬字元（例如再次貼回本系統的輸入框） | 取不回 |

- 它能指出「這段文字或這張圖是誰的裝置留下的」，不能阻止外流，也不能證明是誰按下的快門。
- 沒有介面開關，也沒有額外說明文字；只有浮水印本身可見。
